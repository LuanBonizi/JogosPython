...
class NPC(AnimatedSprite):
    ...
    def movement(self):
        next_pos = self.game.pathfinding.get_path(self.map_position, self.game.player.map_pos)
    ...
    def check_health(self):
        if self.health < 1:
            ...
            self.alive = False
            if self not in self.game.object_handler.dead_npc_list:
                self.game.object_handler.dead_npc_list.append(self)
    ...
    def ray_cast_player_npc(self):
        if self.game.player.map_pos == self.map_position:
            return True
