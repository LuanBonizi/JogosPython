...
class Player:
    ...
    def check_game_over(self):
        if self.health < 1:
            ...
            self.game.object_renderer.game_over_screen()
            pg.display.flip()
            pg.time.delay(1500)
            self.game.new_game()

    def check_victory(self):
        if len(self.game.object_handler.npc_list) == len(self.game.object_handler.dead_npc_list):
            self.game.object_renderer.victory_screen()
            pg.display.flip()
            pg.time.delay(5000)
            self.game.sound.theme.stop()
            self.game.new_game()
